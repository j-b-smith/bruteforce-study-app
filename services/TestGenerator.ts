...
class TestGenerator {
  ...
  private shuffleQuestionOptions(question: Question): Question {
    const shuffledOptions = this.shuffleArray([...question.options]); // Shuffle a copy of the options array
    return {
      ...question,
  ...
  generateCategoryQuiz(courseName: string, categoryName: string): Question[] {
    ...
    const shuffledQuestions = this.shuffleArray([...category.questions]); // Shuffle a copy of the questions array
  ...
  generateCustomQuiz(courseName: string, selectedCategories: string[]): Question[] {
    ...
    const questions = selectedCategories.flatMap(categoryName => {
      const category = course.categories.find(cat => cat.name === categoryName);
      if (!category) throw new Error(`Category ${categoryName} not found in course ${courseName}`);
      return [...category.questions]; // Use a copy of the questions array
    });
